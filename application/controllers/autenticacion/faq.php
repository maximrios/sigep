 ...
 * @copyright 2012-01
 */
class Faq extends Ext_crud_controller {
    ...
    public function index() {
        //
        $this->_vcContentPlaceHolder = $this->load->view('lib_autenticacion/principal-faq', array(), true);
        //		 
        parent::index();
    }
    ...
    protected function _inicReg($boIsPostBack=false) {
        $this->_reg = array(
            'inIdFaq' => null, 'tsFecPublicacion' => null, 'inIdTemaFaq' => null, 'vcPregunta' => '', 'vcRespuesta' => '', 'inIdOrden' => null, 'bActivo' => null
        );
        $inId = ($this->input->post('inIdFaq') !== false) ? $this->input->post('inIdFaq') : 0;
        if ($inId != 0 && !$boIsPostBack) {
            $this->_reg = $this->_oModel->obtenerUno($inId);
            $this->_reg['tsFecPublicacion'] = GetDateFromISO($this->_reg['tsFecPublicacion'], FALSE);
        } else {
            $this->_reg = array(
                'inIdFaq' => $inId
                , 'tsFecPublicacion' => ($this->input->post('tsFecPublicacion') === FALSE) ? GetToday('d/m/Y') : set_value('tsFecPublicacion')
                , 'inIdTemaFaq' => $this->input->post('inIdTemaFaq')
                , 'vcPregunta' => set_value('vcPregunta')
                , 'vcRespuesta' => set_value('vcRespuesta')
