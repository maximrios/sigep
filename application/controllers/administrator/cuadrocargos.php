...
class Cuadrocargos extends Ext_crud_controller {
    ...
    protected function _inicReg($boIsPostBack=false) {
        ...
        if ($inId != 0 && !$boIsPostBack) {
            ...
            $this->_reg = array(
                ...
                , 'perteneceCuadroCargo' => set_value('perteneceCuadroCargo')
            );
        }
        return $this->_reg;
    }
    ...
    public function listado($idNoticia = 0) {
        ...
        $editar = '<a href="administrator/cuadrocargos/formulario/{idCuadroCargo}" title="Editar el estado del cargo Orden N° {ordenCuadroCargo} - {denominacionCargo}" class="btn-accion" rel="{\'idCuadroCargo\': {idCuadroCargo}}"><span class="glyphicon glyphicon-pencil"></span></a>';
        $tamano = 32;
        $acciones = $editar;
    ...
    public function formulario($idCuadroCargo=0) {
        $aData['Reg'] = $this->_inicReg($this->input->post('vcForm'));
        $aData['vcFrmAction'] = 'administrator/cuadrocargos/guardar';
    ...
    public function guardar() {
        ...
        if ($this->_validarReglas()) {
            ...
            $this->_aEstadoOper['status'] = $this->cuadrocargo->guardar(
                    array(
                        ...
                        , $this->_reg['perteneceCuadroCargo']
                    )
            );
        ...
        if ($this->_aEstadoOper['status'] > 0) {
            //redirect('administrator/cuadrocargosagentes');
            $this->listado();
        } 
        else {
            $this->formulario();
