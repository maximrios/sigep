...
class Pases_model extends CI_Model {
    ...
    public function obtener($idActuacion, $vcBuscar = '', $limit = 0, $offset = 9999999) {
        $sql = 'SELECT * 
        FROM sigep_view_actuaciones_pases
        WHERE idActuacion = ?
        AND nombreOrigen LIKE ? 
        ORDER BY fechaEnvioActuacionPase DESC
        LIMIT ? offset ? ;';
        return $this->db->query($sql, array((int) $idActuacion, '%' . strtolower((string) $vcBuscar) . '%', (double) $offset, (double) $limit))->result_array();
    }
    ...
    public function numRegs($idActuacion, $vcBuscar) {
        $sql = 'SELECT count(idActuacionPase) AS inCant FROM sigep_view_actuaciones_pases
        WHERE idActuacion = ? AND nombreOrigen LIKE ?;';
        $result = $this->db->query($sql, array((int) $idActuacion, strtolower('%' . strtolower($vcBuscar) . '%')))->result_array();
