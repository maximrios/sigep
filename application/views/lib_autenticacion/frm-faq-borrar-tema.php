    ...
    <form id="<?= $vcFormName; ?>" name="<?= $vcFormName; ?>" action="<?= $vcFrmAction; ?>" method="post" target="contenido-abm">
        <fieldset>
            ...
            <div class="fila">
                ...
                <textarea name="vcTemaFaq" id="vcTemaFaq"<?= classRegValidation('vcTemaFaq') ?> style="width: 450px; height: 30px; resize: none;"><?= $RegTema['vcTemaFaq']; ?></textarea>
            </div>
            <div class="clearfloat">&nbsp;</div>  
            <div class="fila">
                <label for="vcTemaDesc">Descripcion:</label>
                <textarea name="vcTemaDesc" id="vcTemaDesc"<?= classRegValidation('vcTemaDesc') ?> style="width: 450px; height: 30px; resize: none;"><?= $RegTema['vcTemaDesc']; ?></textarea>
            </div>
            <div class="clearfloat">&nbsp;</div>           
            <div class="fila">
            ...
            <div class="fila">
                ...
                <input name="bActivo" type="checkbox" id="bActivo" value="bActivo" <?= ((bool)$RegTema['bActivo'] == 't') ? 'checked="true"' : ''; ?> />
            </div>
            <div class="clearfloat">&nbsp;</div>
        </fieldset>
        <br/>
        <div class="buttons">
            <input type="submit" class="button guardar btn-accion <?= (empty($RegTema['inIdTemaFaq'])) ? 'btn-reset' : '' ; ?>" value="Eliminar"/>
...
<script type="text/javascript">
    $(document).ready(function(){
        $('#<?= $vcFormName; ?> textarea:first').focus();
    });
</script>
